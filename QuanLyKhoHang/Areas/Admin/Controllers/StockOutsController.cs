    ...
    public class StockOutsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchString)
        {
            ...
            if (!string.IsNullOrEmpty(searchString))
            {
                ...
                query = query.Where(s =>
                    (s.Customer != null && s.Customer.Name.ToLower().Contains(keyword)) || // Tìm theo tên KH
                    s.StockOutDetails.Any(d => d.Product != null && d.Product.Name.ToLower().Contains(keyword)) 
                );
            }

            var stockOuts = await query.OrderByDescending(s => s.Date).ToListAsync();
